...
| `--judge.prompt_preset` | Output parser |
|---|---|
| `meta-eval-pair-score` | PairScore at the meta-eval temperature |
| `arena-hard` | Arena-Hard verdict `[[A>>B]]` … `[[B>>A]]` |
| `meta-eval-alpaca-eval-json` | AlpacaEval `ordered_models` JSON |
| `meta-eval-alpaca-eval-pair-score` | PairScore from the AlpacaEval-style prompt |
