...
Results go under `[result_folder]/<task>-<prompt_preset>-<judge>-<swap_mode>/` as `sample.parquet`, `annotations.parquet`, `summary.csv`, `results.json`, `config.yaml`, and `run-metadata.v1.json`. `results.json` reports agreement on all battles and on the subset that drops human ties, English vs multilingual ranking splits, held-out Elo MAE vs annotation budget (`elo_gap_all` and `elo_gap_exclude_ties`), and a `telemetry` block with token and cost estimates. Token counts are `chars/4`. USD uses a local `openrouter_pricing.json` under `JUDGEARENA_DATA/cache/` when present; nothing is fetched at runtime, and missing pricing writes the cost fields as null.
